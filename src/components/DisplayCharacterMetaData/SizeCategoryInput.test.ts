...
describe('Size category input field', () => {
  ...
  beforeEach(() => {
    onChangeHandler = jest.spyOn(CharacterMetadataActions, 'setCharacterSizeCategory')
  ...
  it('should render successfully', () => {
    expect(characterMetadataForm.getByTestId(SizeCategoryInputTestIds.wrapper)).toBeInTheDocument()
  })

  it('should render a select element with 3 children', () => {
    const select = characterMetadataForm.getByTestId(SizeCategoryInputTestIds.select)
  ...
  describe('should call the onChange handler', () => {
    it('with small when the first option is clicked', () => {
      expectClickOnNthButtonToSetValue(
        characterMetadataForm, onChangeHandler, 0, SizeCategory.SMALL, SizeCategoryInputTestIds.select)
    })

    it('with medium when the second option is clicked', () => {
      expectClickOnNthButtonToSetValue(
        characterMetadataForm, onChangeHandler, 1, SizeCategory.MEDIUM, SizeCategoryInputTestIds.select)
    })

    it('with large when the third option is clicked', () => {
      expectClickOnNthButtonToSetValue(
        characterMetadataForm, onChangeHandler, 2, SizeCategory.LARGE, SizeCategoryInputTestIds.select)
    })
  })
